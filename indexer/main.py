...
def parse_register_resolver_message(message: str) -> dict:
    ...
    register_resolver_events = [
        ...
        if event.get("type") == register_resolver_event_name
    ]
    ...
    return dict(
        map(
            lambda event: (
                event.get("iri").replace('"', ""),
                int(event.get("id").replace('"', "")),
            ),
            register_resolver_events,
        )
    )
...
def decode_event_attribute(attribute: dict) -> tuple:
    return base64.b64decode(attribute.get("key")).decode("utf-8"), base64.b64decode(
        attribute.get("value")
    ).decode("utf-8")
...
def get_resolver_url(resolver_id: int) -> str:
    if resolver_id == settings.REGEN_RESOLVER_ID and settings.REGEN_RESOLVER_ALIAS:
        return settings.REGEN_RESOLVER_ALIAS
...
def fetch_resolver_url(resolver_id: int) -> str:
    api_url = urljoin(
        settings.REGEN_NODE_REST_URL, f"regen/data/v1/resolver/{resolver_id}"
    )
